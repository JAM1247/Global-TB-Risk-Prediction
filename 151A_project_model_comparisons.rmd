...
tb_data <- wdi_data %>%
  filter(!is.na(TB)) %>%
  dplyr::select(country, region, iso2c, iso3c, year, TB, HIV, GDP, HEC, PPD, URB, SMP, SAN, IMR) %>%
  dplyr::mutate(
    log_TB  = log(TB),
    log_GDP = log(GDP),
    log_HEC = log(HEC),
...
cat("Dataset contains", nrow(tb_data), "rows and has been saved to your folder.\n")
```
...
full_model <- lm(log(TB) ~ HIV + log_GDP  + log_HEC + log_PPD + URB + SMP + SAN + IMR + region, data = train)
summary(full_model)
```
...
But there are deviations in the tails → minor violation of normality.
